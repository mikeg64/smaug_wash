...
int addsourceterms2_MODID(real *dw, real *wd, real *w, struct params *p, struct state *s,int *ii,int field,int dir) {
  ...
  int status=0;
   ...
   real expfac, exp_x,exp_y,exp_z,exp_xyz;
   ...
   // xc2_i=0.0;
    ...
    //xc3=xc3_i+(qt-t0)*(xc3_0-xc3_i)/t0;
     xc2=xc2_i;  //moves in x2 direction
     xc3=xc3_i;
    ...
    dz=0.125e6;
        ...
        exp_x=1.0/(exp(expfac)+(1.0/exp(expfac)));
        exp_x=exp_x*exp_x; //compute sech^2 here i.e. a soliton
