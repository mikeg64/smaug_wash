  ...
  int status=0;
   ...
   // xc2_i=0.0;
    ...
    dz=0.125e6;
       ...
       // tdec=(qt<t0?exp(-4*(qt-t0)*(qt-t0)/(t0*t0)):1);
        tdec=(exp(-4*(qt-t0)*(qt-t0)/(t0*t0)));
      ...
      //  vy=aa*exp_xyz*tdepy;
       ...
       w[fencode3_MODID(p,ii,mom2)]+=(p->dt)*av*(w[fencode3_MODID(p,ii,rhob)]+w[fencode3_MODID(p,ii,rho)]);
