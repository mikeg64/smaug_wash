...
directory='/storage2/mikeg/results/blast3d/';
...
ndirectory='/storage2/mikeg/results/blast3d/images_3d_vsecs/';
nextension='.jpg';
...
id=int2str(1000*i);
filename=[directory,'zeroOT_',id,extension];
...
%TP=(gamma-1.d0).*TP;
  ...
  h=slice(myval,64, 64,[5 49 100]);  %used for 0,1 mode
  ...
  %colormap(cmap);
  hc=colorbar();
  set(hc,'Ylim',[-1500 1500]);
  %set(hc,'Ylim',[-0.6 0.6]);
  %set(hc,'Ylim',[4*10^5 3*10^6]);
  text(-100,0,165,timetext);
  title('Vertical Velocity for Solar Atmosphere with a Sinusoidal (0,0) Mode Driver of Period 300s, Applied at a Height of 100km');
