...
sabx=reshape(wd(6,nrange,nrange,nrange)+wd(11,nrange,nrange,nrange),124,124,124);
saby=reshape(wd(7,nrange,nrange,nrange)+wd(12,nrange,nrange,nrange),124,124,124);
sabz=reshape(wd(8,nrange,nrange,nrange)+wd(13,nrange,nrange,nrange),124,124,124);
% TP=reshape(wd(9,nrange,nrange,nrange)+wd(5,nrange,nrange,nrange),124,124,124);
TP=((sabx.*rootmu).^2.0+(saby.*rootmu).^2.0+(sabz.*rootmu).^2.0)./2.0;
% TP=TP-(val1.^2+val2.^2+val3.^2)./(2.*val4);
% TP=(gamma-1.d0).*TP;
 ...
 %myval=shiftdim(mu_gas.*TP./R,1); %computes the temperature 
 myval=shiftdim(TP,1); %computes the pressure   
